...
const ProfileInfo = ({ children }: Props) => (
...
export const Profile = Object.assign(ProfileRoot, {
  ...
  Name: ({ children }) => <h3 className="font-bold text-lg">{children}</h3>,
  Role: ({ children }) => <p className="text-sm text-gray-500">{children}</p>,
});
...
const ProfileRoot = ({ children }: Props) => {
  const childrenArray = Children.toArray(children) as React.ReactElement[];

  // Wyłapujemy konkretne komponenty "po typie"
  const avatarSlot = childrenArray.find(child => child.type === ProfileAvatar);
  const infoSlot = childrenArray.find(child => child.type === ProfileInfo);
...
Ten pattern pozwala na ignorowanie nieznanych dzieci lub wyrzucanie błędów, jeśli wymagane sloty nie są dostarczone.
